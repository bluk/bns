...
/// BNSHTTP2Stream is a HTTP 2.x stream.
public final class BNSHTTP2Stream: BNSHTTPStream,
    BNSStartable,
    BNSCancellable,
    BNSReadableStream,
    BNSWritableStream,
    ...
    internal var eventLoopProtectedState: BNSStreamInternalState<BNSHTTP2Stream>
        = BNSStreamInternalState<BNSHTTP2Stream>()
    ...
    internal lazy var tryReadyTransitionFromStart: () -> Void = { [weak self] in
        ...
        if self.isRequestHeadSet || self.isPushRequest {
            self.logTrace("Request head was already set or this is a push request so transitioning to ready state.")
    ...
    public private(set) var connection: BNSHTTP2Connection
    /// The stream ID
    public let streamID: HTTP2StreamID

    internal var sendBuffer: BNSSendBuffer<BNSHTTP2Stream>
        = BNSSendBuffer<BNSHTTP2Stream>()
    internal var receiveBuffer: BNSHTTPReceiveBuffer<BNSHTTP2Stream>
        = BNSHTTPReceiveBuffer<BNSHTTP2Stream>()

    private var isRequestHeadSet: Bool
    private var isPushRequest: Bool
    ...
    internal init(
        ...
        self.isRequestHeadSet = false
        self.isPushRequest = isPushRequest
        self.streamID = h2StreamID
    }
    ...
    internal final func requestBegin(requestHead: HTTPRequestHead) {
        guard self.eventLoop.inEventLoop else {
            self.eventLoop.execute { self.requestBegin(requestHead: requestHead) }
            return
        }

        assert(!self.isPushRequest, "Push request should not see a requestHead.")

        self.logDebug("Read request head: \(requestHead)")

        self.withQueueIfPossible {
            assert(self.requestHead == nil, "Request head should not have been set.")
            self.requestHead = requestHead
            self.responseHead = HTTPResponseHead(version: requestHead.version, status: .ok)
        }

        self.isRequestHeadSet = true

        switch self.eventLoopProtectedState.state {
        case .preparing:
            self.logTrace("Request head was already set or this is a push request so transitioning to ready state.")
            self.eventLoopProtectedState.transition(to: .ready, on: self)
            self.receiveBuffer.process(on: self)
            self.sendBuffer.process(on: self)
        case .setup, .ready, .failed, .cancelled:
            return
        }
    }
}
